...
// 3D cursor interaction for book with less aggressive angles
document.addEventListener('DOMContentLoaded', function() {
...
// Handle book rotation based on mouse position with gentler angles
function handleBookRotation(e, book, container) {
    ...
    // Calculate rotation angles based on mouse position, but with gentler effects
    // Limit rotation to smaller values
    const rotateY = Math.min(Math.max(-8 + (mouseX / rect.width * 5), -10), -3);
    const rotateX = Math.min(Math.max(2 - (mouseY / rect.height * 2), 0), 4);
...
function resetBookRotation(book) {
    book.style.transform = 'rotateY(-8deg) rotateX(2deg)';
}
...
function turnPage(pageId) {
    ...
    if (activePage) {
        ...
        }, 400); // Slightly reduced for smoother transitions
    }
}
